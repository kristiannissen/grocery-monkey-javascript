...
const Groceries = () => {
  ...
  const handleDelete = (e) => {
    setGroceries(
      groceries.filter((g) => e.target.parentElement.parentElement.dataset.key !== g.id)
    );
  };
    ...
    <div className="min-w-full">
      <div className="min-w-full">
        {groceries.map((grocery) => (
          <div className="flex flex-row h-12" key={grocery.id} data-key={grocery.id}>
            <div className="basis-3/4 px-1.5 py-1.5">{grocery.name}</div>
            <div className="basis-1/4 px-1.5 py-1.5 justify-end">
      <span className="absolute top-0 bottom-0 right-0 px-4 py-3" onClick={handleDelete}>
        <svg
